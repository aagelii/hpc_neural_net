...
Layer::Layer(int size_kernel, int channels, int input_size)
{
	...
	// for forward pass
	hipMalloc(&output, sizeof(float) * input_size);
	hipMalloc(&pact, sizeof(float) * input_size);
	hipMalloc(&bias, sizeof(float) * channels);
	hipMalloc(&weight, sizeof(float) * size_kernel * channels);

	// for backward pass
	hipMalloc(&back_out, sizeof(float) * input_size);
	hipMalloc(&back_pact, sizeof(float) * input_size);
	hipMalloc(&back_weight, sizeof(float) * size_kernel * channels);

	// Copy data from CPU to GPU
	hipMemcpy(bias, host_bias, sizeof(float) * channels, hipMemcpyHostToDevice);
	hipMemcpy(weight, host_weight, sizeof(float) * size_kernel * channels, hipMemcpyHostToDevice);
}
...
{
	hipMemcpy(output, data, sizeof(float) * input_size, hipMemcpyHostToDevice);
}

// To clear out previous data from previous iterations, for forward pass
void Layer::clear()
{
	hipMemset(output, 0, sizeof(float) * input_size);
	hipMemset(pact, 0, sizeof(float) * input_size);
}

// To clear out previous data from previous iterations, for backward pass
void Layer::backprop_clear()
{
	hipMemset(back_out, 0, sizeof(float) * input_size);
	hipMemset(back_pact, 0, sizeof(float) * input_size);
	hipMemset(back_weight, 0, sizeof(float) * size_kernel * channels);
}
...
__global__ void bkwd_bias_conv(float bias[6], float back_pact[6][24][24])
{
	...
	for (int idx = N * thread_pos / grid_size; 
		...
		const int idx1 = (idx % 6);
		const int idx2 = ((idx /= 6) % 24);
		const int idx3 = ((idx /= 24) % 24);
